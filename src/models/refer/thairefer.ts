...
export class ThaiReferModel {
  ...
  person(db: Knex, columnSearch, valueSearch) {
    return db("person").where(columnSearch, valueSearch).limit(maxLimit);
  }

  referOut(db: Knex, columnSearch, valueSearch) {
    let sql = db("referout")
      ...
      .select(
        ...
        "cc as CHIEFCOMP",'memodiag as DIAGLAST',
  ...
  sumReferOut(db: Knex, columnSearch, valueSearch) {
    let sql = db("referout")
      .select(`hcode`)
      .count('hn as cases')
      .where(columnSearch, valueSearch)
  ...
  getPerson(db: Knex, columnName, searchText, hospCode = hcode) {
    //columnName = cid, hn
    //columnName = columnName === 'cid' ? 'hn' : columnName;
    return db("patient as pt")
      ...
      .select(
        ...
        "pt.road as road",
        "addr_soi as soi",
        "pt.addressid as addcode",
        "pt.hometel as tel",
        "pt.po_code as zip",
        "op.nhso_code as occupation"
      )
