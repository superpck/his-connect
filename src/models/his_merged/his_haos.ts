...
export class HisHaosModel {
    ...
    async getDiagnosisOpdVWXY(db: Knex, date: any) {
        let query = db('ovstdiag as dx')
            ...
            .select('hn', 'vn as visitno', 'dx.vstdate as date', 'icd10 as diagcode',
                'icd.name as diag_name', 'dx.diagtype as diag_type', 'doctor as dr',
                'dx.episode', db.raw(`? as codeset`, ['IT']), 'update_datetime as d_update')
            .whereIn('vn', function () {
    ...
    async getAdmission(db: Knex, columnName: string, searchValue: any, hospCode = hcode) {
    ...
    async getAdmission_(db: Knex, columnName, searchValue, hospCode = hcode) {
        columnName = columnName === 'an' ? 'i.an' : columnName;
        columnName = columnName === 'hn' ? 'i.hn' : columnName;
        columnName = columnName === 'visitNo' ? 'q.vn' : columnName;
        columnName = columnName === 'dateadmit' ? 'i.regdate' : columnName;
