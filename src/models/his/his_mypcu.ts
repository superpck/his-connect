...
export class HisMyPcuModel {
    check() {
        return true;
    }

    testConnect(db: Knex) {
        return db('send_person').select('HN').limit(1)
    }
    ...
    getReferOut(db: Knex, date, hospCode = hcode) {
        return db('m_service as service')
            ...
            .select('service.REFEROUTHOSP as HOSP_DESTINATION'
                , 'service.CAUSEOUT', 'service.SEQ as REFERID'
                , 'service.CHIEFCOMP', 'service.PDX as DIAGLAST'
                , 'service.REFERINHOSP as HOSPCODE_ORIGIN'
                , 'service.ROWID as REFERID_ORIGIN'
            )
    ...
    getPerson(db: Knex, columnName, searchText, hospCode = hcode) {
        ...
        return db('send_person as p')
            .select('p.HOSPCODE', 'p.CID', 'p.PID', 'p.HID'
                , 'p.PRENAME', 'p.NAME', 'p.LNAME'
                , 'p.HN', 'p.SEX'
                , 'p.MSTATUS', 'p.OCCUPATION_OLD', 'p.OCCUPATION_NEW'
                , 'p.RACE', 'p.NATION', 'p.RELIGION'
                , 'p.EDUCATION', 'p.FSTATUS', 'p.FATHER'
                , 'p.MOTHER', 'p.COUPLE', 'p.VSTATUS'
                , 'p.DISCHARGE'
                , 'p.ABOGROUP', 'p.RHGROUP', 'p.LABOR'
                , 'p.PASSPORT', 'p.TYPEAREA')
            // , 'p.ID', 'p.TELEPHONE', 'p.MOBILE')
    ...
    getAddress(db: Knex, columnName, searchText, hospCode = hcode) {
        ...
        return db('m_address as a')
            .select(`a.HOSPCODE`, `a.PID`
                , 'a.ADDRESSTYPE', 'a.HOUSE_ID'
                , 'a.HOUSETYPE', 'a.ROOMNO', 'a.CONDO'
                , 'a.HOUSENO', 'a.SOISUB', 'a.SOIMAIN'
                , 'a.ROAD', 'a.VILLANAME', 'a.VILLAGE'
                , 'a.TAMBON', 'a.AMPUR', 'a.CHANGWAT'
    ...
    getService(db: Knex, columnName, searchText, hospCode = hcode) {
        ...
        return db('m_service')
            .select('service.HOSPCODE', 'service.PID',
                'service.HN', 'service.SEQ',
                'service.LOCATION', 'service.INTIME',
                'service.INSTYPE', 'service.INSID',
                'service.MAIN', 'service.TYPEIN',
                'service.REFERINHOSP', 'service.CAUSEIN',
                'service.CHIEFCOMP', 'service.FAMILYHISTORY',
                'service.PRESENTILLNESS', 'service.PASTHISTORY',
                'service.PHYSICALEXAM', 'service.SERVPLACE',
                'service.WEIGHT', 'service.HEIGHT',
                'service.BTEMP', 'service.SBP',
                'service.DBP', 'service.PR',
                'service.RR', 'service.TYPEOUT',
                'service.REFEROUTHOSP', 'service.CAUSEOUT',
                'service.COST', 'service.PRICE',
                'service.PAYPRICE', 'service.ACTUALPAY',
                'service.ID',
                'service.SUB', 'service.HSUB',
                'service.CID')
    ...
    getDiagnosisOpd(db: Knex, visitNo, hospCode = hcode) {
        return db('m_diagnosis_opd')
            .where('SEQ', visitNo);
    }

    getProcedureOpd(db: Knex, visitNo, hospCode = hcode) {
    ...
    getDrugOpd(db: Knex, visitNo, hospCode = hcode) {
        return db('m_drug_opd')
            .where('SEQ', visitNo);
    }

    getAdmission(db: Knex, columnName, searchNo, hospCode = hcode) {
        return [];
    }
    ...
    getChargeIpd(db: Knex, an, hospCode = hcode) {
        return [];
    }
    ...
    getAccident(db: Knex, visitNo, hospCode = hcode) {
    ...
    getReferHistory(db: Knex, columnName, searchNo, hospCode = hcode) {
        columnName = columnName === 'visitNo' ? 'service.SEQ' : columnName;
        columnName = columnName === 'vn' ? 'service.SEQ' : columnName;
        columnName = columnName === 'hn' ? 'service.PID' : columnName;
        columnName = columnName === 'pid' ? 'service.PID' : columnName;
        ...
        return db('m_service as service')
            ...
            .select('service.REFEROUTHOSP as HOSP_DESTINATION'
                , 'service.CAUSEOUT', 'service.SEQ as REFERID'
                , 'CONCAT(service.SEQ,service.HOSPCODE) as REFERID_PROVINCE'
                , 'service.CHIEFCOMP', 'service.PDX as DIAGLAST'
                , 'service.REFERINHOSP as HOSPCODE_ORIGIN'
                , 'service.ROWID as REFERID_ORIGIN'
            )
