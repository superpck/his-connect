...
class HisKpstatModel {
    ...
    getPerson(db, columnName, searchText, hospCode = hcode) {
        ...
        return db('mrls.pt')
            ...
            .select('pt.idpop as cid', 'pt.hn as pid', 'pt.pname as prename', 'pt.fname as name', 'lname', 'pt.hn', 'mate as sex', 'brthdate as birth', 'mrtlst as mstatus')
    ...
    getAddress(db, columnName, searchNo, hospCode = hcode) {
        ...
        return db('mrls.pt')
            .select(db.raw('"' + hcode + '" as hospcode'))
            .select('hn as pid', 'housetype', 'addrpart as houseno', 'moopart as village', 'tmbpart as tambon', 'amppart as ampur', 'chwpart as changwat', 'd_update', 'idpop as cid')
    ...
    getService(db, columnName, searchText, hospCode = hcode) {
        ...
        return db('ovst')
            ...
            .select(db.raw('"' + hcode + '" as hospcode'))
            .select('ovst.hn as pid', 'ovst.vstno as visitno', 'ovst.vstno as seq', 'ovst.vstdate as date_serv', 'vsttime as time_serv', 'ovst.intime', 'ovst.nopoor as insid')
            ...
            .select('rfrin.rfrics as causein', 'ovst.nurse as chiefcomp', 'ovst.ovstplace as servplace', 'ovst.temp1 as btemp', 'ovst.bp1 as sbp', 'ovst.bp2 as dbp', 'ovst.pulse as pr', 'ovst.rate as rr', 'ovst.ovstost as typeout')
    ...
    getDiagnosisOpd(db, visitno, hospCode = hcode) {
        return db('ovstdiag as dx')
            ...
            .select('ovst.hn as pid', 'dx.vstno as seq', 'ovst.vstdate as date_serv', 'dx.dxtype as diagtype', 'dx.icd10 as diagcode')
    ...
    getReferOut(db, date, hospCode = hcode) {
        return db('mrls.rfrout as refer')
            ...
            .select('refer.rfrono as referid', 'refer.hn', 'pt.idpop as cid', 'refer.vstno as seq', 'refer.an', 'pt.pname as prename', 'pt.fname', 'pt.lname', 'pt.brthdate as dob', 'pt.mate as sex', 'refer.icd10 as dx')
...
exports.HisKpstatModel = HisKpstatModel;
