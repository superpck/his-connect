...
class HisHaosModel {
    ...
    async getDiagnosisOpdVWXY(db, date) {
        let query = db('ovstdiag as dx')
            ...
            .select('hn', 'vn as visitno', 'dx.vstdate as date', 'icd10 as diagcode', 'icd.name as diag_name', 'dx.diagtype as diag_type', 'doctor as dr', 'dx.episode', db.raw(`? as codeset`, ['IT']), 'update_datetime as d_update')
            .whereIn('vn', function () {
            this.select('vn').from('ovstdiag as dx2')
    ...
    async getVisitForMophAlert(db, date, isRowCount = false, startRow = -1, limit = 100) {
        ...
        if (isRowCount) {
            const [row] = await baseQuery.clone().count({ row_count: 1 });
            return row || { row_count: 0 };
        }
        const dataQuery = baseQuery
            .clone()
            .select(db.raw(`'10679' as hospcode`))
            .select('p.IdCard as cid', 'p.hn as hn', 'a.ServiceId as vn', 'w.Mapping1 as department_code', 'w.Name as department_name')
