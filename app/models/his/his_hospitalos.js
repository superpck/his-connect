    ...
    async testConnect_(db) {
        ...
        try {
            if (clientType.includes('mysql')) {
                ...
                const result = await db.raw('SELECT collation_name AS charset FROM sys.databases WHERE name = ?', [process.env.HIS_DB_NAME]);
                charset = result?.recordset?.[0]?.charset || '';
            }
            else if (clientType.includes('oracledb')) {
                const result = await db.raw("SELECT value AS charset FROM nls_database_parameters WHERE parameter = 'NLS_CHARACTERSET'");
