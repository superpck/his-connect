...
exports.PccHisJhcisModel = void 0;
const dbName = process.env.HIS_DB_NAME;
const maxLimit = 500;
class PccHisJhcisModel {
    check() {
        return true;
    }
    getTableName(knex) {
    ...
    getService(db, hn, date) {
        return db('visit')
            .select('pid', 'pid as hn', 'visitno', 'visitdate as date', 'timestart as time')
    ...
    getServiceByHn(db, hn, cid, date = '', visitno = '') {
        ...
        return db('visit')
            ...
            .select('visit.pcucode as hospcode', 'visit.*', 'visit.pid as hn', 'visit.visitdate as date', 'visit.visitdate as date_serv', 'visit.timestart as time')
            .select(db.raw("case when LOCATE('/', pressure) then SUBSTR(pressure,1,LOCATE('/', pressure)-1) else '' end as bp_systolic"))
            .select(db.raw("case when LOCATE('/', pressure) then SUBSTR(pressure,LOCATE('/', pressure)+1) else '' end as bp_diastolic"))
    ...
    getDrug(db, visitNo) {
        return db('visitdrug as drug')
            ...
            .leftJoin('chospital', 'visit.pcucode', 'chospital.hoscode')
            .select('drug.*', 'drug.pcucode as hospcode', 'chospital.hosname as hospname', 'lib.drugname', 'visit.visitdate as date_serv', 'visit.timestart as time_serv', 'lib.pack', 'lib.unitsell as unit_sell', 'lib.unitusage as unit_use', 'lib.cost', 'lib.sell as price', 'lib.drugcaution as caution', 'lib.drugcode24 as code24', 'lib.tcode', 'lib.tmtcode as tmt', 'lib.drugproperties as comment')
    ...
    getDrugByHn(db, pid) {
        return db('visitdrug as drug')
            ...
            .leftJoin('cdrug as lib', 'drug.drugcode', 'lib.drugcode')
            .leftJoin('chospital', 'visit.pcucode', 'chospital.hoscode')
            .select('drug.*', 'drug.pcucode as hospcode', 'chospital.hosname as hospname', 'lib.drugname', 'visit.visitdate as date_serv', 'visit.timestart as time_serv', 'lib.pack', 'lib.unitsell as unit', 'lib.unitusage as unit_use', 'lib.cost', 'lib.sell as price', 'lib.drugcaution as caution', 'lib.drugcode24 as code24', 'lib.tcode', 'lib.tmtcode as tmt', 'lib.drugproperties as comment')
            .where('visit.pid', "=", pid)
            .where('lib.drugtype', "=", '01')
            .orderBy('visit.visitdate', 'desc')
            .orderBy('visit.timestart', 'desc');
    ...
    getChronic(db, pid, cid) {
        ...
        return db('personchronic as chronic')
            ...
            .select('chronic.*', 'chronic.pcucodeperson as hospcode', 'chospital.hosname as hospname')
...
exports.PccHisJhcisModel = PccHisJhcisModel;
